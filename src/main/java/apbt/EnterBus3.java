 ...
 * @author Sam
 */
public class EnterBus3 {

    int busMAX = 12;
    Lock bus_lock = new ReentrantLock();

    public void bus(RunFunction rF, Room1 room1Count, Room2 room2Count, Room3 room3Count, Bus3 bus3Count, Bus3Arrive bus3Arrive, CheckForTicket checkForTicket, LeaveTerminal leaveTerminal, InspectorToiletBreakCheck inspectorToiletBreakCheck, InWaitingRoom inWaitingRoom) {
        try {

            if (bus3Count.count == busMAX) {
                System.out.println("                                                                                BUS 3 FULL & LEFT");

                synchronized (bus3Arrive) { //Concurrency concept - synchronization deployed to avoid race condition.
                    bus3Arrive.arrived = false;
                    bus3Arrive.wait = false;
                }

                synchronized (bus3Count) {  //Concurrency concept - synchronization deployed to avoid race condition.
                    bus3Count.count = 0;
                }
                synchronized (this) {   //Concurrency concept - synchronization deployed to notify bus 3 arrive function.
                    this.notify();
                }
            }
                                ...
                                if (option == 0) {
                                    if (room1Count.count != 0) {
                                        ...
                                        synchronized (room1Count) { //Concurrency concept - synchronization deployed to avoid race condition.
                                            room1Count.count--;
                                            System.out.println(rF.getName() + "                                     entered BUS 3");
                                            synchronized (checkForTicket) { //Concurrency concept - synchronization deployed to avoid race condition.
